-- Sprint 3: speed_roll_markets opens 5m + 1m (1h killed in mig 0040).
--
-- 1m markets open every minute on the boundary. Cron fires every 5s; the
-- "skip if open future market exists" check naturally throttles to one new
-- 1m market per minute boundary.
--
-- Body byte-equal to canonical 0040 except v_durations array.
