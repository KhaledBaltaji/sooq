-- Mig 0061: _speed_pricing_apply — new compression curve gated by feature flag.
--
-- When fee_config.speed_use_new_curve = 1:
--   - Offered prob computed via min-cap curve so offered -> 1 / 0 smoothly
--   - soft_blocked always returned FALSE (gate removed)
--
-- When fee_config.speed_use_new_curve = 0 (default):
--   - Existing linear formula: offered = mark + widened_spread/2
--   - soft_blocked computed as before (>= soft_block_threshold from market_config)
--
-- All other logic (matrix lookup, neighbor fallback, asym push-up, telemetry
-- INSERT) is unchanged so the two paths produce identical mark_prob.
...
DECLARE
  ...
  v_half_spread       DOUBLE PRECISION;
...
BEGIN
  ...
  IF p_mode = 'entry' THEN
    -- ── PRICING CURVE BRANCH ────────────────────────────────────────────
    IF v_use_new_curve = 1 THEN
      -- Mig 0061: compression curve. Half-spread caps at proportion of
      -- distance-to-edge so offered can never exceed 1 (or fall below 0).
      -- Mark side >= 0.5 → push offered UP toward 1.
      -- Mark side  < 0.5 → push offered DOWN toward 0 (still a positive
      --                    house edge: offered <= mark on the loser side).
      v_half_spread := p_widened_spread / 2.0;
      IF v_final_mark_prob >= 0.5 THEN
        v_final_offered := v_final_mark_prob
          + LEAST(v_half_spread, (1.0 - v_final_mark_prob) * 0.5);
      ELSE
        v_final_offered := v_final_mark_prob
          - LEAST(v_half_spread, v_final_mark_prob * 0.5);
      END IF;
      -- soft-block stays FALSE under new curve (gate removed by design).
      v_blocked := FALSE;
    ELSE
      -- Legacy linear formula (mig 0034 behavior).
    ...
    END IF;
  ELSE
    -- Cashout mode: offered_prob unused. Return mark for symmetry.
    v_final_offered := v_final_mark_prob;
  END IF;

  -- Numerical safety floor/cap. Under new curve these should never bind
  -- (formula already keeps offered in (0, 1)); under legacy they preserve
  -- the existing 0.01 / 0.99 behavior.
  ...
  -- Telemetry: insert is best-effort. Same event row format as mig 0036+.
  -- Under new curve, soft_blocked column is always FALSE (the gate is gone).
  BEGIN
...
COMMENT ON FUNCTION public._speed_pricing_apply(text, speed_duration, text, double precision, double precision, double precision, double precision, text, uuid) IS
  $$0061 Phase 5: dual-mode pricing. When speed_use_new_curve=1, uses compression curve offered = mark +/- min(half_spread, (1-mark)*0.5 or mark*0.5). When =0, legacy linear formula with soft-block at threshold. Flag default 0 — admin flips to activate.$$;
