-- Mig 0061 Phase 5: speed_execute_cashout — parity bucket ±1 tolerance.
--
-- Only change vs mig 0034+0051: the seconds_left_bucket parity check now
-- accepts ±1 instead of strict equality. Same fix as trade RPC (silent
-- failure #7 mitigation) — kills the 30s/60s cliff that rejected
-- legitimate 1-2s click latency on cashouts.
--
-- Cashout near-decided block (line ~207 in original) is intentionally
-- NOT made sticky — cashout volume is 13% of trade volume on staging
-- and the gate fires far less than the soft-block. Plus making cashout
-- sticky would trap user stakes for the rest of the round, which is
-- the opposite of trust-building. Founder may revisit later.
...
COMMENT ON FUNCTION public.speed_execute_cashout(uuid, text, numeric, numeric, integer, numeric, numeric) IS
  $$0061 Phase 5: seconds_left_bucket parity tolerance ±1 (mirrors trade RPC). All other logic unchanged from mig 0051 Phase 2C body.$$;
