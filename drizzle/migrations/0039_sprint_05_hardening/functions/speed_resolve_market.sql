-- S0.9: settlement boundary fix — `<` becomes `<=`.
--
-- BUG (pre-0039): IF NOW() < closes_at THEN RAISE 'Market has not closed yet'.
-- At exactly NOW() = closes_at (microsecond window), resolution is allowed.
-- Strictly speaking, settlement should require closes_at to have STRICTLY
-- passed; matching closes_at is "the moment of close" not "after close".
-- Tiny correctness window but worth fixing.
--
-- All other logic byte-equal to canonical 0021.
...
GRANT EXECUTE ON FUNCTION public.speed_resolve_market(uuid) TO sooqadmin;
