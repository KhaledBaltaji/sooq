-- Mig 0062 Phase 5e: speed_execute_cashout — per-market cashout gate.
--
-- After loading v_mc (per-market config), check v_mc.cashout_enabled.
-- When FALSE (currently: all 1m markets), raise a clean message before
-- any pricing or margin work. Other markets are unchanged.
--
-- The mig 0061 changes (parity bucket ±1) are preserved verbatim.
...
BEGIN
  ...
  -- ── 0062 Phase 5e: per-market cashout gate ────────────────────────────
  -- Closes the longshot-extract exploit on 1m markets. The frontend reads
  -- the same flag from /api/speed/quote and renders a passive position
  -- monitor instead of a cashout button, so users never see this error in
  -- the normal flow. This is the server-side authoritative check.
  IF v_mc.cashout_enabled = FALSE THEN
    RAISE EXCEPTION 'Cashout isn''t available on this market — hold to settlement.'
      USING HINT = format('asset=%s duration=%s', v_market.asset, v_market.duration);
  END IF;
  -- ──────────────────────────────────────────────────────────────────────
  ...
  -- 0061 Phase 5: parity bucket tolerance ±1 (mirrors speed_execute_trade).
  ...
  IF p_expected_seconds_left_bucket IS NOT NULL
    ...
    RAISE EXCEPTION 'PARITY_DRIFT [seconds_left_bucket]: expected=% actual=% (drift > 1 bucket)',
      ...
      USING HINT = 'Cashout window changed substantially — refresh quote';
  END IF;
...
COMMENT ON FUNCTION public.speed_execute_cashout(uuid, text, numeric, numeric, integer, numeric, numeric) IS
  $$0062 Phase 5e: per-market cashout gate via speed_market_config.cashout_enabled. 1m markets are disabled to close the longshot-extract exploit. All other logic preserved from mig 0061.$$;
