-- Sprint 4 Phase 5A (mig 0052): speed_execute_trade gets:
--   1. Per-asset oracle_stale_seconds from speed_asset_config (BTC stays
--      2s, gold gets 30s — handles thin PAXG bookticker during off-hours)
--   2. GOLD-1m branch in payout cap cascade (before existing 1m / 5m / 1h)
--
-- Body otherwise byte-equal to canonical post-0051.
...
BEGIN
  ...
  -- 0052 Phase 5A: per-asset oracle_stale_seconds from speed_asset_config.
  -- BTC: NULL → falls back to fee_config global (2s). GOLD: 30s for thin books.
  ...
  -- 0051 Phase 2C: new helper signature takes asset+duration
  ...
  ELSIF v_market.asset = 'GOLD' AND v_market.duration::TEXT = '1m' THEN
    -- 0052 Phase 5A: GOLD-1m branch (only fires if speed_market_config row missing)
