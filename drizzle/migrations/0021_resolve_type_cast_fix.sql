-- 0021_resolve_type_cast_fix.sql
--
-- HOTFIX: speed_resolve_market raised
--   "operator does not exist: speed_side = text"
-- on every non-at_strike resolution that had at least one position. The
-- comparison `v_pos.side = v_outcome::TEXT` mixes a `speed_side` enum and
-- text without an explicit cast on the LHS — Postgres rejects it.
--
-- speed_resolve_expired_markets caught the per-market exception in its
-- loop, so pg_cron always returned `success=true` even though the inner
-- `failed` counter was non-zero. Markets stayed status='open' and user
-- positions stayed status='open' indefinitely. Cron fired ~17 280 ×/day
-- with `failed >= 1` for hours before this was spotted by the deep audit.
--
-- Fix: cast both sides to TEXT in the comparison. No other changes.
-- All other behaviour in mig 0016 §6 (audit row, NGR cache, advisory lock,
-- exact-tick + wick detector, void path) is untouched.
--
-- After applying, the next speed-resolve cron beat (≤5s) will pick up all
-- stuck markets and settle them. Affected positions get the correct payout
-- per the rules in mig 0016: winners receive ROUND(stake / entry_offered_prob, 2).
...
GRANT EXECUTE ON FUNCTION public.speed_resolve_market(UUID) TO PUBLIC;
