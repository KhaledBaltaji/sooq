-- S0.2: NULL guards on aggregate cap SUM queries.
--
-- BUG (pre-0038): two SUM(stake / entry_offered_prob) clauses in the per-side
-- and strike-cluster cap checks (lines ~377, ~397 of the canonical body).
-- A single historical row with NULL or zero entry_offered_prob crashes the
-- division and aborts the trade with a cryptic DB error. Trades fail silently
-- for legitimate users until ops finds the bad row.
--
-- FIX: WHERE entry_offered_prob IS NOT NULL AND entry_offered_prob > 0 on
-- both aggregations. Body is otherwise byte-for-byte the canonical 0034
-- definition; this is a surgical CREATE OR REPLACE.
