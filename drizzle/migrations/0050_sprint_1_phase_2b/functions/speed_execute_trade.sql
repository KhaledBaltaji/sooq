-- Sprint 1 Phase 2B (mig 0050): speed_execute_trade reads remaining
-- per-(asset, duration) values from speed_market_config.
--
-- Phase 2A (mig 0049) did spread_pct + soft_block_threshold (the activation
-- blockers). Phase 2B does the rest of the trade-side reads:
--   - last_n_reject_secs (was speed_late_window_reject_s)
--   - cap_per_side_usd (was speed_cap_per_side_usd)
--   - near_decided_dist (was speed_late_30s_imbalance_reject)
--   - late_window_30s_mult (was speed_late_30s_spread_mult)
--   - late_window_60s_mult (was speed_late_60s_spread_mult)
--   - per_side_pool_pct (was speed_max_market_exposure_pct)
--   - payout_max_usd (was speed_entry_max_payout_usd_<5m|1m|gold_5m> cascade)
--
-- Pattern: SELECT * INTO v_mc at RPC entry (after v_market load). Each
-- per-market read becomes COALESCE(v_mc.column, fee_config.global_key, default).
-- Single snapshot at RPC entry per F1.3 (mid-trade race avoidance).
--
-- Phase 2C (next): speed_execute_cashout + helper sig changes
-- (_speed_max_stake_for_offered, _speed_cashout_margin) for the remaining
-- cashout/stake reads.
--
...
DECLARE
  ...
  v_mc                 speed_market_config%ROWTYPE;  -- 0050 Phase 2B: per-market config snapshot
...
BEGIN
  ...
  -- 0050 Phase 2B: load per-(asset, duration) config once at RPC entry.
  -- Single snapshot per F1.3 (avoid mid-trade race against admin UPDATE).
  -- Empty row tolerated; downstream reads COALESCE through fee_config fallback.
  ...
  -- 0050 Phase 2B: last_n_reject_secs from market_config first
  ...
  -- 0050 Phase 2B: cap_per_side_usd from market_config first
  ...
  IF v_seconds_left < 30 THEN
    -- 0050 Phase 2B: near_decided_dist from market_config first
  ...
  IF v_seconds_left < 30 THEN
    -- 0050 Phase 2B: late_window_30s_mult from market_config first
  ...
  ELSIF v_seconds_left < 60 THEN
    -- 0050 Phase 2B: late_window_60s_mult from market_config first
  ...
  -- 0050 Phase 2B: payout_max_usd from market_config replaces the per-(asset, duration)
  -- fee_config cascade. Cascade kept as fallback if market_config row missing.
  ...
  -- 0050 Phase 2B: per_side_pool_pct from market_config first
