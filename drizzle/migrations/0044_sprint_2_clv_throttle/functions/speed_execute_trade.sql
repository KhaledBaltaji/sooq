-- Sprint 2: wire _speed_apply_user_shading into speed_execute_trade.
--
-- Shading is the LAST pricing layer (after matrix + asym push-up + soft-block
-- + floor/cap). Pushes offered_prob UP for users with reliably positive edge.
-- The shaded value is what gets stored in speed_positions.entry_offered_prob,
-- so settlement and cashout naturally use it (Option A coupling per Phase 4 plan).
--
-- Changes vs canonical 0039 body:
--   1. Read speed_entry_soft_block_threshold once (so shading helper respects
--      admin-tuned threshold).
--   2. After floor/cap, call _speed_apply_user_shading(user, offered, threshold).
--   3. If shading raised offered_prob, set v_shading_applied = TRUE for parity skip.
--   4. Update parity-skip condition: skip if matrix pushed up OR shading applied.
--   5. Track shading delta in the return JSON for observability.
