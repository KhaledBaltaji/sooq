-- ============================================================================
-- 354_speed_rv_freshness.sql
--
-- Tighten realized-volatility (RV) staleness window from 5 minutes to 90
-- seconds across all callsites: speed_execute_trade, speed_execute_cashout,
-- get_speed_volatility (client RPC).
--
-- Codex flagged: "RV cache is accepted if fresher than 5 minutes, while price
-- freshness is 2 seconds. That means price can be fresh while sigma is stale."
-- In a fast BTC move, the model uses old volatility for up to 5 minutes —
-- looks live, isn't live. Mispricing.
--
-- Fix: tighten the window to 90 seconds (3x the cache refresh cadence of 30s,
-- so two refresh misses in a row are still tolerable). DRY the staleness
-- check via a new helper `_speed_get_iv(asset)` so the same window applies
-- everywhere automatically.
--
-- This migration also re-applies mig 353's exposure cap fix to
-- speed_execute_trade — the function body must be re-emitted in full because
-- plpgsql doesn't support partial updates.
-- ============================================================================

-- ── 1. Helper: read RV cache with 90s freshness, fall back to fee_config ─────

CREATE OR REPLACE FUNCTION public._speed_get_iv(p_asset public.speed_asset)
RETURNS DECIMAL
...
DECLARE
  v_iv DECIMAL;
BEGIN
  -- Fresh cache (within 90 seconds) → return cached RV
  SELECT rv INTO v_iv
  FROM speed_realized_vol_cache
  WHERE asset = p_asset
    AND computed_at > NOW() - INTERVAL '90 seconds';

  -- Stale or missing → fall back to static IV from fee_config
...
COMMENT ON FUNCTION public._speed_get_iv(public.speed_asset) IS
'Mig 354: shared IV lookup. Reads speed_realized_vol_cache with 90s freshness window (was 5min); falls back to fee_config.speed_iv_btc on stale/missing. Used by speed_execute_trade, speed_execute_cashout, get_speed_volatility.';

GRANT EXECUTE ON FUNCTION public._speed_get_iv(public.speed_asset) TO anon, authenticated, service_role;

-- ── 2. Update get_speed_volatility (client RPC) to use 90s window ────────────

CREATE OR REPLACE FUNCTION public.get_speed_volatility(p_asset public.speed_asset)
RETURNS JSONB
...
BEGIN
  ...
  -- Mig 354: tightened freshness from 5min to 90s. Matches _speed_get_iv.
...
COMMENT ON FUNCTION public.get_speed_volatility(public.speed_asset) IS
'Mig 352 (Seam 4) + Mig 354: public RPC for client volatility badge. Returns {rv, computed_at, source} as jsonb. source=cache when speed_realized_vol_cache is fresh (<90s); source=fallback when stale/empty (returns fee_config.speed_iv_btc).';

-- ── 3. Update speed_execute_trade to use the helper (carries mig 353 fix) ────
...
BEGIN
  ...
  -- Mig 354: tightened RV staleness from 5min to 90s via shared helper.
  ...
  -- ── Per-market aggregate exposure cap (mig 353 fix carried forward) ───
...
'Place a speed bet. Three-flow dispatch (retail / commission-branch / reseller-branch). Mig 352 (Seams 3+4): IV from speed_realized_vol_cache (fallback fee_config.speed_iv_btc). Mig 353: exposure cap proxy fix — replaces p_stake*2 with true SUM(stake/offered_prob) per-side payout liability. Mig 354: tightened RV staleness from 5min to 90s via _speed_get_iv helper. Mig 345 locking semantics preserved.';

-- ── 4. Update speed_execute_cashout to use the helper ────────────────────────
...
'Cash out a single open speed position. Mig 352 (Seam 4): IV from speed_realized_vol_cache. Mig 351: continuous cashout multiplier. Mig 354: tightened RV staleness from 5min to 90s via _speed_get_iv helper. Mig 345: main-pool sentinel locking.';
