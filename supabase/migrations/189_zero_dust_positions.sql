-- ============================================================
-- 189: Zero out dust positions after sell
--
-- Bug: floating-point arithmetic in execute_trade can leave tiny
-- residual shares_held (e.g. 0.0000000001) after a full close.
-- The frontend query (.gt("shares_held", 0)) includes these dust
-- positions, displaying "0.000 lots" with misleading P&L.
--
-- Fix: After the sell UPDATE, zero out positions where shares_held
-- dropped below 0.001 (< 0.000001 lots — worth < $0.001).
-- Also one-time cleanup of existing dust positions.
-- ============================================================

-- One-time cleanup of existing dust positions
UPDATE positions SET shares_held = 0, total_invested = 0
WHERE shares_held > 0 AND shares_held < 0.001;

-- Recreate execute_trade with dust cleanup after sell
