-- 126_fn_resolve_market_v2.sql — Updated resolve_market for NGR commission model
-- Changes from v1 (migration 113):
--   1. Calls settle_resolution_commissions() instead of settle_commissions()
--   2. Calls record_revenue() with no commission parameter (it queries directly)
-- All other logic identical to migration 113.
...
BEGIN
  ...
  -- ======= [CHANGED] SETTLE RESOLUTION COMMISSIONS =======
  -- Trade-time commissions already paid per-trade. This only handles resolution fee commissions.
  v_total_commissions := settle_resolution_commissions(p_market_id);

  -- ======= [CHANGED] RECORD REVENUE (no commission parameter) =======
