-- 179: Fix resolve_market — admin_id → admin_user_id
--
-- Migration 176 used wrong column name (admin_id) for admin_config table.
-- The actual column is admin_user_id. This re-applies the corrected function.
-- Also adds SET search_path and trigger_bypass for SECURITY DEFINER safety.
-- search_path includes extensions for pgcrypto (crypt/gen_salt).
