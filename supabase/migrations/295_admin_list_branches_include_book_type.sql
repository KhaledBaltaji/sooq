-- ============================================================
-- 295: Commission Branch — Phase 2 hotfix #2
--
-- Context: /admin/branches list page needs to show a Type column to
-- distinguish reseller from commission branches (and future types).
-- admin_list_branches currently doesn't return book_type, so the UI
-- has no way to display the distinction.
--
-- Fix: extend the SELECT inside admin_list_branches to include b.book_type.
-- Same signature, same auth, same shape otherwise — clients that don't
-- read book_type just ignore the extra field.
--
-- Caught during post-push staging review when admin reported "I don't see
-- branch type when creating" — the underlying list view has the same gap.
-- ============================================================
