-- 125_fn_execute_trade_v2.sql — NGR commission integration
-- Changes from v1 (migration 110):
--   1. Deterministic lock ordering: trader + all ancestors locked in UUID order
--   2. Commission hook: PERFORM pay_trade_commissions() after each trade INSERT
-- All other logic is identical to migration 110.
...
DECLARE
  ...
  -- [NEW] Referral chain for deterministic locking
  v_chain UUID[];
BEGIN
  -- ======= AUTH =======
  v_user_id := auth.uid();
  ...
  -- [NEW] Read referral chain and lock ALL ancestors in deterministic UUID order
  -- This prevents deadlocks when two referred users of the same agent trade concurrently
  ...
  IF p_direction = 'buy' THEN
    ...
    -- 12. [NEW] Real-time commission payment
  ...
  ELSIF p_direction = 'sell' THEN
    ...
    -- [NEW] Real-time commission payment
