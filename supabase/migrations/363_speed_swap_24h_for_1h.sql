-- ============================================================================
-- 363_speed_swap_24h_for_1h.sql
--
-- Reverse mig 361's choice. After shipping 5m + 24h on staging, the user
-- decided 24h doesn't fit the "speed market" identity (too slow, no game
-- feel) and switched the offering to 5m + 1h. This migration:
--   1. Updates speed_roll_markets() durations: ['5m','24h'] → ['5m','1h']
--   2. Voids any pending/open 24h markets via the same refund flow mig 361
--      used for 15m/1h. On staging this catches 2 markets with 0 positions;
--      on production it'll catch any active 24h positions and refund cleanly.
--
-- 1h is "revived" by simply being back in the durations array — speed_roll
-- starts queuing pending 1h rows at the next cron tick. The 47 historical
-- resolved 1h markets remain queryable.
--
-- 15m enum value remains unused but in place (47 + 3 historical rows still
-- reference it). 24h enum value also remains in place (47 + 3 historical
-- rows). Same reasoning as mig 361: dropping enum values would break
-- historical SELECTs.
-- ============================================================================
...
BEGIN
  ...
  -- Mig 363: swapped '24h' for '1h'. Active offering is 5m + 1h.
...
'Mig 363: swapped 24h for 1h. Active offering is 5m + 1h. Historical 15m/24h rows remain queryable.';

-- ── Void any currently-pending or open 24h markets ─────────────────────
-- Inlines the same logic mig 361 used for 15m/1h: refund stake to user,
-- pool ledger entry, settlements row, notification, market voided.
DO $$
DECLARE
  v_market         RECORD;
  v_pos            RECORD;
...
BEGIN
  FOR v_market IN
    ...
    WHERE duration = '24h'::speed_duration
  ...
  LOOP
    ...
    LOOP
      ...
      VALUES (v_pos.user_id, 'speed_refund', v_pos.stake, v_new_balance, v_pos.id,
              'Speed market voided -- 24h discontinued (mig 363)');
      ...
      ELSE
        ...
        VALUES (NULL, v_market.id, 'refund', -v_pos.stake, v_new_pool, v_pos.id,
                'Refund from main pool (24h dropped -- mig 363)');
      END IF;
    ...
    SET status = 'voided', voided_at = NOW(),
        void_reason = 'Mig 363: 24h dropped -- switched to 1h',
        updated_at = NOW()
  ...
  RAISE NOTICE 'Mig 363: voided % 24h markets, refunded % positions', v_voided_count, v_refund_count;
END;
$$;
