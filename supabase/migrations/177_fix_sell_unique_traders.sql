-- 177: Fix execute_trade sell path
--
-- Two fixes:
-- 1. Sell path now recalculates unique_traders (was only on buy path)
-- 2. Restore price_impact_warning in return object (was dropped in 173)
...
BEGIN
  ...
  ELSIF p_shares_to_sell IS NOT NULL AND p_shares_to_sell > 0 THEN
    ...
    -- FIX: sell path now recalculates unique_traders (was missing, only on buy path)
    UPDATE markets SET
      trade_count = trade_count + 1,
      unique_traders = (SELECT COUNT(DISTINCT user_id) FROM trades WHERE market_id = p_market_id)
    WHERE id = p_market_id;
