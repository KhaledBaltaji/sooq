-- ============================================================
-- Fix: execute_trade references fee_config.key/value but table
-- has fee_type/rate columns. Also adds missing V3 fee rows.
-- ============================================================

-- 1. Insert V3-specific fee rows if missing
INSERT INTO fee_config (fee_type, rate, description)
VALUES
  ('explicit_fee', 0.005, 'V3: 0.5% explicit trading fee')
ON CONFLICT (fee_type, level, depth) DO NOTHING;

INSERT INTO fee_config (fee_type, rate, description)
VALUES
  ('cash_out_premium', 0.005, 'V3: 0.5% cash-out premium on sells')
ON CONFLICT (fee_type, level, depth) DO NOTHING;

INSERT INTO fee_config (fee_type, rate, description)
VALUES
  ('max_trade_pct', 0.10, 'Max trade size as % of volume + $100')
ON CONFLICT (fee_type, level, depth) DO NOTHING;

INSERT INTO fee_config (fee_type, rate, description)
VALUES
  ('dynamic_spread_threshold', 0.05, 'Price threshold for dynamic spread')
ON CONFLICT (fee_type, level, depth) DO NOTHING;

INSERT INTO fee_config (fee_type, rate, description)
VALUES
  ('dynamic_spread_multiplier', 0.50, 'Dynamic spread multiplier')
ON CONFLICT (fee_type, level, depth) DO NOTHING;

-- 2. Recreate execute_trade with correct column references (fee_type/rate)
...
BEGIN
  ...
  -- Bypass protected columns trigger (we are SECURITY DEFINER)
  ...
  -- 4. Read fee config (using correct column names: fee_type/rate)
