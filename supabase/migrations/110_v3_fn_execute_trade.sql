-- 110_v3_fn_execute_trade.sql — Core trading function (buy + sell)
-- Replaces V2 place_bet. Uses auth.uid(), SELECT FOR UPDATE, fee_config.
-- p_amount = USD for buys, share count for sells.
...
BEGIN
  ...
  -- ======= LOCK ORDER: users → markets → amm_state =======
  ...
  IF p_direction = 'buy' THEN
    ...
    IF v_current_price > v_dyn_threshold THEN
      -- Extra spread when buying into the already-heavy side
    ...
    -- 5. Calculate AMM spread cost (for revenue tracking, not charged separately)
    -- AMM spread = actual cost - theoretical cost at flat pre-trade price
    ...
    v_price_per_share := v_net_amount / v_shares;  -- LMSR cost per share (excludes fees, stays < 1.0)
  ...
  ELSIF p_direction = 'sell' THEN
    ...
    -- Validate sell doesn't exceed AMM q values (prevents negative q)
    ...
    -- AMM spread for tracking (theoretical vs actual)
    v_current_price := CASE WHEN p_side = 'yes'
      THEN v_amm.current_yes_price ELSE v_amm.current_no_price END;
    ...
    -- Credit user balance (sells do NOT update total_wagered)
    ...
    -- Update position: reduce shares, track realized P&L
