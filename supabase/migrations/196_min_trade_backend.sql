-- ============================================================
-- 195: Enforce minimum trade amount on buys (backend guard)
--
-- The frontend enforces MIN_TRADE = $5, but the backend had no
-- check. Anyone calling the RPC directly could submit sub-$5
-- buys. Sells/closing positions have no minimum — users can
-- always exit.
-- ============================================================

-- Add min trade amount to fee_config (configurable, not hardcoded)
INSERT INTO fee_config (fee_type, rate, description)
VALUES ('min_trade_amount', 5, 'Minimum buy trade amount in USD')
ON CONFLICT (fee_type, level, depth) DO NOTHING;

-- Recreate execute_trade with minimum trade guard on buy path
