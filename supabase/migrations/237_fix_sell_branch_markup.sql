-- Migration 237: Fix sell path — branch_markup should be 0, not exit_fee
-- Migration 236 inherited from 212 where branch_markup stored exit_fee for sells.
-- Now that exit_fee_amount is a dedicated column, branch_markup should be 0 for sells.
-- Only the sell INSERT line changes; reapply full function.
...
BEGIN
  ...
  ELSIF p_shares_to_sell IS NOT NULL AND p_shares_to_sell > 0 THEN
    ...
    -- branch_markup = 0 for sells (no markup on cash-out), exit_fee_amount captures the fee
    INSERT INTO branch_trades (
      trade_id, branch_id, agent_id, user_id, market_id,
