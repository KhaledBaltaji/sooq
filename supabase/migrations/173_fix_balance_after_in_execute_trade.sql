-- ============================================================
-- 173: Fix balance_after in execute_trade transactions
--
-- Bug: INSERT INTO transactions used v_user.balance_usd (pre-trade
-- snapshot) instead of the computed post-trade balance. Every other
-- function (resolve_market, process_deposit, etc.) correctly records
-- the post-update balance. This was the only function with the bug.
--
-- Fix: Compute post-trade balance inline:
--   BUY:  v_user.balance_usd - p_amount
--   SELL: v_user.balance_usd + v_net_proceeds
-- ============================================================
...
BEGIN
  ...
  IF p_amount IS NOT NULL AND p_amount > 0 THEN
    ...
    -- FIX: balance_after now correctly shows post-trade balance (was using pre-trade snapshot)
