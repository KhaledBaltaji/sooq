-- ============================================================
-- 220: Make resolve_market, record_revenue, settle_resolution_commissions branch-aware
--
-- Changes:
-- 1. resolve_market: filter winner payouts to retail only (branch_id IS NULL),
--    call branch_settle_resolution after retail, filter seed_pnl/stats/notifications
-- 2. record_revenue: use retail_trades/retail_positions views to avoid double-counting
-- 3. settle_resolution_commissions: filter to retail positions only
-- ============================================================

-- ============================================================
-- 1. resolve_market — branch-aware
-- ============================================================
...
BEGIN
  ...
  -- Check for winning positions (ALL positions, including branch)
  ...
  -- ======= SEED P&L (RETAIL ONLY — use retail_trades view) =======
  ...
  -- ======= NOTIFY RETAIL POSITION HOLDERS (winners + losers) =======
  -- Winners (retail only)
  ...
  WHERE p.market_id = p_market_id AND p.side = p_outcome AND p.shares_held > 0
    AND p.branch_id IS NULL;  -- *** RETAIL ONLY ***
...
-- ============================================================
-- 2. record_revenue — retail only (prevents branch double-counting)
-- ============================================================
...
DECLARE
  ...
  v_market RECORD;
BEGIN
  -- *** RETAIL ONLY: use retail_trades view to exclude branch trades ***
  SELECT
  ...
  -- Resolution fee: from winning RETAIL positions only
  SELECT * INTO v_market FROM markets WHERE id = p_market_id;

  IF v_market.outcome IS NOT NULL THEN
    -- *** RETAIL ONLY: use retail_positions view ***
  ...
  -- Query total commissions from referral_commissions (retail only — branch commissions don't exist yet)
  SELECT COALESCE(SUM(commission_amount), 0) INTO v_total_commissions
  FROM referral_commissions
  WHERE market_id = p_market_id AND status = 'credited';

  -- Get total volume (this is shared AMM volume — kept as-is for now)
  ...
  -- Insert revenue record (retail only — no branch_id)
...
-- ============================================================
-- 3. settle_resolution_commissions — retail only
-- ============================================================
...
BEGIN
  ...
  -- *** RETAIL ONLY: filter to branch_id IS NULL ***
  -- This prevents branch winners from triggering retail referral commissions
  FOR v_pos IN
