-- Fix: execute_trade needs to set app.trigger_bypass so the
-- prevent_sensitive_user_updates trigger allows balance_usd changes.
...
BEGIN
  ...
  ELSIF p_shares_to_sell IS NOT NULL AND p_shares_to_sell > 0 THEN
    ...
    UPDATE markets SET bet_count = bet_count + 1 WHERE id = p_market_id;
  ...
  WHERE market_id = p_market_id AND triggered = false
    AND (
