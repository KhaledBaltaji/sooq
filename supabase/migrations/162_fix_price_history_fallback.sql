-- 162_fix_price_history_fallback.sql — Fix get_price_history RPC
-- Issues fixed:
-- 1. Fallback used current AMM price instead of initial LMSR price (0.5)
--    for buckets before the first trade.
-- 2. "ALL" period now starts from the first trade (not market creation)
--    to avoid long flat 50% dead zones when trading started late.
...
BEGIN
  -- Find the first trade time for this market (used for "ALL" period start
  -- and to avoid generating excessive pre-trade buckets)
  ...
  CASE p_period
    ...
    WHEN 'ALL' THEN
      -- Start from first trade (with small padding) instead of market creation
      -- to avoid long 50% dead zones when trading started late
      IF v_first_trade IS NOT NULL THEN
        v_start := v_first_trade - INTERVAL '5 minutes';
      ELSE
        v_start := COALESCE(p_created_at, v_end - INTERVAL '1 day');
      END IF;
      v_interval := GREATEST(
        (v_end - v_start) / 500,
        INTERVAL '1 minute'
