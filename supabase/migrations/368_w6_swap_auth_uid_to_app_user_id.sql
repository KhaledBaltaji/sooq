-- ============================================================================
-- 368_w6_swap_auth_uid_to_app_user_id.sql
--
-- W6 — Replaces speed_execute_trade and speed_execute_cashout with versions
-- that call app.user_id() (W6 GUC pattern) instead of auth.uid() (Supabase).
--
-- Generated by sedding migration 366. Re-runs CREATE OR REPLACE for the same
-- 3 RPCs (speed_execute_trade, speed_execute_cashout, speed_resolve_market —
-- the third one had no auth.uid() refs and is no-op-replaced for parity).
-- The ALTER TABLE speed_settlements DROP COLUMN IF EXISTS branch_id is
