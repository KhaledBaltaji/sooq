-- ============================================================
-- 274: Block trading on upcoming (pre-launch) markets
--
-- Problem: admin-created markets carry status = 'open' from birth, but
-- `opens_at` may be scheduled in the future. execute_trade only checked
-- `status = 'open'` and `now() < closes_at`, so a direct RPC could trade
-- a scheduled market before its launch time. The UI now gates trading on
-- `opens_at`, but the backend must also enforce it.
--
-- Fix: add `now() < opens_at` guard to:
--   1. public.execute_trade (live) — copies body from migration 173
--   2. public.demo_execute_trade (demo sandbox) — copies body from 271
--
-- Error text uses "market not open yet" so the client error mapper in
-- src/lib/trade-utils.ts can route it to i18n key `errorMarketUpcoming`.
-- ============================================================

BEGIN;

-- ═══════════════════════════════════════════════════════════════════════════
-- 1. execute_trade (live)
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- [274] Pre-launch guard: market scheduled for future must not be tradable.
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 2. demo_execute_trade — same guard for demo sandbox
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Inline patch: we don't redefine the whole demo function here. Instead we
-- wrap the existing body by adding the same opens_at check as the first thing
-- after locking the demo market row. Executed via DO so the rest of the
-- 271-defined function stays authoritative.

-- Drop+recreate only the demo variant header needed to reuse migration 271's
-- body would be brittle. Instead we CREATE OR REPLACE the full demo function
-- with the guard. Body mirrors 271 exactly except for the one added line.
...
DECLARE
  ...
  v_patched TEXT;
BEGIN
  -- Pull the current function body, inject the opens_at guard right after the
  -- existing `Market has closed` check. If the source already contains the
  -- guard (re-run safety), skip.
  SELECT pg_get_functiondef(p.oid)
    INTO v_src
  FROM pg_proc p
  JOIN pg_namespace n ON n.oid = p.pronamespace
  WHERE n.nspname = 'public'
    AND p.proname = 'demo_execute_trade'
    AND pg_get_function_identity_arguments(p.oid)
        = 'p_market_id uuid, p_side text, p_amount numeric, p_shares_to_sell numeric';

  IF v_src IS NULL THEN
    RAISE EXCEPTION 'demo_execute_trade not found — cannot apply opens_at guard';
  END IF;

  IF v_src LIKE '%Market not open yet%' THEN
    RAISE NOTICE 'demo_execute_trade already guarded';
    RETURN;
  END IF;
  ...
  v_patched := replace(
    v_src,
    'IF now() >= v_market.closes_at THEN RAISE EXCEPTION ''Market has closed''; END IF;',
    'IF now() < v_market.opens_at THEN RAISE EXCEPTION ''Market not open yet''; END IF;' || E'\n' ||
    '  IF now() >= v_market.closes_at THEN RAISE EXCEPTION ''Market has closed''; END IF;'
  );

  IF v_patched = v_src THEN
    RAISE EXCEPTION 'demo_execute_trade body did not match expected pattern — refusing to patch';
  END IF;
