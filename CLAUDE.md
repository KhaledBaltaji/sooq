...
- **Oracle architecture (Group B, 0017):** EC2 worker at `services/speed-oracle/` subscribes to `wss://stream.binance.com:9443/ws/btcusdt@trade` (10-30 ticks/sec for BTC) and writes throttled at 10 Hz to `speed_oracle_latest` + `speed_oracle_ticks`. Frontend reads via Binance WS DIRECTLY (`useBinanceTicker` → `lib/binance/ws-client.ts`) for sub-second chart smoothness; falls back to `/api/speed/oracle` polling if Binance WS unreachable from the user's network. Trade execution + settlement still read `speed_oracle_latest` from the DB inside their RPCs — server is always the source of truth. Public health endpoint at `/api/health/oracle` reports oldest tick age; cron at `.github/workflows/oracle-monitor.yml` alerts on >5s staleness. Worker host: EC2 `i-03411906c55af48af` (`63.183.214.217`), systemd unit `speed-oracle.service`. Wick detector threshold bumped 0.001 → 0.003 in mig 0017 to account for raw @trade noise vs @kline_1s.
